...
class Customer_Info_List_Table extends WP_List_Table {
       ...
       foreach ($users as $each) {
		    ...
		    if(!empty($customer_orders)){
			    ...
			    foreach($customer_orders as $single){
			    	...
			    	$order .= '<a href="'.get_edit_post_link($single->ID).'" alt="">#'.$single->ID.'</a>';
       		...
       		$newdata[] = array(
       				...
       				'customerid' => $metas['my_unique_id'][0]
	...
	function get_columns() {
		...
		$columns = array(
			...
			'customerid' => __( 'Customer ID', 'customer-info' ),
	...
	function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'customerid':
				...
				return (!empty($data['customerid'])) ? $data['customerid'] : '' ;
